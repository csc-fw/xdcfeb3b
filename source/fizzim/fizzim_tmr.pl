...
#
# $Id: fizzim.pl,v 4.44 2014/02/18 23:43:35 pzimmer Exp pzimmer $ 
...
foreach $output (sort keys %{ $globals{outputs} }) {
  ...
  ($out = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
  if (length($out) > $max_output_name_length) {
    $max_output_name_length = length($out);
  }
  if($flags{$output}) {
    $out = $out . "voted_";
    if (length($out) > $max_flag_name_length){
      $max_flag_name_length = length($out);
    }
  }
  if ($max_output_name_length > $max_flag_name_length) {
    $max_outflag_name_length = $max_output_name_length;
  } else {
    $max_outflag_name_length = $max_flag_name_length;
  }
  # Make table of outputs with suppressed portlists
  if ($globals{outputs}{$output}{useratts} =~ /suppress_portlist/){
    $suppressed_outputs{$output} = 1;
  }
}
...
");
  ...
  if ($globals{outputs}{$output}{useratts} =~ /suppress_portlist/){
    ($output_tmp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
    foreach $sup_out ( keys %suppressed_outputs){
      ($sup_out_tmp = $sup_out) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
      next if ($output_tmp eq $sup_out_tmp);
      if ( $output_tmp =~ $sup_out_tmp ){
        &error($indent,"
Cannot use suppressed portlist output names that are subsets of other suppressed portlist output names or flag names. Violation is $output_tmp and $sup_out_tmp");
      }
    } 
    foreach $flg_out ( keys %flags){
      ($flg_out_tmp = $flg_out) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
      next if ($output_tmp eq $flg_out_tmp);
      if ( $output_tmp =~ $flg_out_tmp ){
        &error($indent,"
Cannot use suppressed portlist output names that are subsets of other suppressed portlist output names or flag names. Violation is $output_tmp and $flg_out_tmp");
      }
    } 
  }
  if ($globals{outputs}{$output}{type} eq "flag") {
    ($output_tmp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
    foreach $sup_out ( keys %suppressed_outputs){
      ($sup_out_tmp = $sup_out) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
      next if ($output_tmp eq $sup_out_tmp);
      if ( $output_tmp =~ $sup_out_tmp ){
        &error($indent,"
Cannot use flag names that are subsets of other flag names or suppressed portlist output names. Violation is $output_tmp and $sup_out_tmp");
      }
    } 
    foreach $flg_out ( keys %flags){
      ($flg_out_tmp = $flg_out) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
      ...
      if ( $output_tmp =~ $flg_out_tmp ){
        &error($indent,"
Cannot use flag names that are subsets of other flag names or suppressed portlist output names. Violation is $output_tmp and $flg_out_tmp");
      }
    } 
  }
  ...
  $line = &create_wirereg_statement("output ",$output,",",$io_statement_length);
  if($TMR){
    # With TMR all outputs are "wires" so change any "reg's" to "wires"
    $line =~ s/^(.*)reg/\1wire/;
  }
  &print($indent,$line);
}
...
if($TMR && $err_det){
  if ($language eq "verilog") {
    &print($inident, "output wire [15:0] $tmr_err_count,\n");
  } elsif ($language eq "systemverilog") {
    &print($inident, "output logic [15:0] $tmr_err_count,\n");
  }
}
...
# if TMR option is set
# setup synthesis attributes and replication suffixes to be used.
if($TMR){
  @tmr_sfx = ("_1", "_2", "_3");
  if($synth_attr eq "XST"){
    if($language eq "verilog"){
      $tmr_reg_attr = "(* equivalent_register_removal = \"no\" *) ";
      $tmr_sig_attr = "(* keep = \"true\" *) "; # The keep attribute in XST does not preserve replication
    }
  } elsif($synth_attr eq "synplify") {
    if($language eq "verilog"){
      $tmr_reg_attr = "(* syn_preserve = \"true\" *) ";
      $tmr_sig_attr = "(* syn_keep = \"true\" *) ";
    }
  } elsif($synth_attr eq "precision") {
    if($language eq "verilog"){
      $tmr_reg_attr = "(* preserve_driver = \"true\" *) ";
      $tmr_sig_attr = "(* preserve_signal = \"true\" *) ";
    }
  } else {
      print STDERR "You must specify a target synthesis tool if you use the -TMR option.\n";
      print STDERR "Use -synth_attr <XST | synplify | precision>\n";
      print STDERR "The logic replication will be optimized away by the synthesis tool\n";
      print STDERR "unless the proper attributes are set on the replicated signals.\n";
      exit;
  }
} else {
  # with no TMR attributes and suffixes are null
  @tmr_sfx = ("");
  $tmr_reg_attr ="";
  $tmr_sig_attr ="";
}

# Find max attribute length for formatting
$max_attr_length = length($tmr_reg_attr);
if(length($tmr_sig_attr) > $max_attr_length) {
  $max_attr_length = length($tmr_sig_attr);
}

if($TMR && $language eq "systemverilog"){
  &print($indent,"$tmr_reg_attr\n");
}
if ($language eq "verilog") {
...
if ($language eq "verilog") {
  ...
  if($TMR){
    #if TMR is used the state variables must be replicated
    &print(--$indent, "} ${statevar}$tmr_sfx[0], ${statevar}$tmr_sfx[1], ${statevar}$tmr_sfx[2], ${nextstatevar}$tmr_sfx[0], ${nextstatevar}$tmr_sfx[1], ${nextstatevar}$tmr_sfx[2];\n");
  } else {
...
foreach $att ("stateout", "nextstateout") {
  # Set TMR attributes: stateout is sequential, nextstateout is combinatorial
  if($att eq "stateout") {
    $tmr_attr = $tmr_reg_attr;
  } else {
    $tmr_attr = $tmr_sig_attr;
  }
  ($var = $att) =~ s/out//;  # turns "stateout" into "state"
  $var = eval "\$${var}var"; # makes var = $statevar
  $stateout = $globals{"machine"}{$att}{"value"};
  if ($stateout eq $var  && !$TMR) {
    # If stateout is set the same as $statevar, then the reg statement was 
    # done in the module header.  Do nothing. (unless TMR replication is needed)
  } else {
    ...
    # Or TMR option was set and we need to replicate the state variables
    if ($language eq "verilog") {
      # if TMR option is set loop over replication suffix
      foreach $sfx (@tmr_sfx) {
        # suppress [0:0]
        if ($last_statebit == 0) {
          &print($indent, "${tmr_attr}reg ${var}$sfx;\n");
        } else {
          &print($indent, "${tmr_attr}reg [$last_statebit:0] ${var}$sfx;\n");
        }
      }
      &print(0, "\n");
    } elsif ($language eq "systemverilog") {
    ...
    # If TMR option is set the voted state variables must be declared before
    # the stateout assignment
    # also add voting logic
    # Voting is only done on sequential logic (stateout but not nextstateout)
    if($TMR && $att eq "stateout"){ 
      if ($language eq "verilog" || $language eq "systemverilog") {
        foreach $sfx (@tmr_sfx) {
          # suppress [0:0]
          if ($last_statebit == 0) {
            &print($indent, "${tmr_sig_attr}$type voted_${var}$sfx;\n");
          } else {
            &print($indent, "${tmr_sig_attr}$type [$last_statebit:0] voted_${var}$sfx;\n");
          }
	}
        &print(0, "\n");
	if($err_det) {
	  # Add replicated signals detected errors
          foreach $sfx (@tmr_sfx) {
              &print($indent, "${tmr_sig_attr}$type err_det_${var}$sfx;\n");
	  }
          &print(0, "\n");
	  # Add replicated error counters for detected errors
          foreach $sfx (@tmr_sfx) {
            &print($indent, "${tmr_reg_attr}reg [15:0] ed_cnt$sfx;\n");
	  }
          &print(0, "\n");
	  # Add replicated voted signals for error counters
          foreach $sfx (@tmr_sfx) {
            &print($indent, "${tmr_sig_attr}wire [15:0] voted_ed_cnt$sfx;\n");
	  }
          &print(0, "\n");
        }
        foreach $sfx (@tmr_sfx) {
          # Add voting logic
          &print($indent, &vote("voted_",$var,$sfx));
	}
        &print(0, "\n");
	if($err_det) {
          foreach $sfx (@tmr_sfx) {
            # Add error detection logic
            &print($indent, &errdet("err_det_",$var,$sfx));
	  }
          &print(0, "\n");
          foreach $sfx (@tmr_sfx) {
            # Add voting for error counters
            &print($indent, &vote("voted_","ed_cnt",$sfx));
	  }
          &print(0, "\n");
        }
      }
    }
    # Now assign the stateout variable
    if($stateout eq ""){
      # No attribute set - Do nothing
    }else {
      if($TMR){
        if ($language eq "verilog" || $language eq "systemverilog") {
          &print($indent, "assign $stateout = voted_${var}$tmr_sfx[0];\n");
        }
      }else {
        if ($language eq "verilog" || $language eq "systemverilog") {
    ...
    if($TMR && $err_det && $att eq "stateout") {
	&print($indent, "assign $tmr_err_count = voted_ed_cnt$tmr_sfx[0];\n");
    }
    &print(0, "\n");
  }
}
...
# reg lines for all outputs if TMR option
#######################
foreach $output (sort keys %{ $globals{outputs} }) {
  if($TMR){
    # TMR case, replicate all outputs
    next if($globals{outputs}{$output}{type} eq "reg"); # Skip "reg" outputs, declared in module statement and will have continuous assignment to <voted_state_1> 
    foreach $sfx (@tmr_sfx) {
#    foreach $sfx ("", @tmr_sfx) {
#      next if($sfx eq "" && !($globals{outputs}{$output}{useratts} =~ /suppress_portlist/)); # Skip statement for original outputs declared in module as ports 
#      next if($sfx eq "" && ($globals{outputs}{$output}{type} eq "flag")); # Skip statement for original flags 
      &print($indent,&create_tmr_wirereg_statement("",$sfx,$output,$max_io_length + length("reg ")));
    }
    if($globals{outputs}{$output}{type} eq "flag" || ($globals{outputs}{$output}{useratts} =~ /suppress_portlist/)){
      # flags and suppressed outputs get an additional "voted" combinatorial signal replicated
      foreach $sfx (@tmr_sfx) {
        &print($indent,&create_tmr_wirereg_statement("voted_",$sfx,$output,$max_io_length + length("wire ")));
      }
    }
  } else {
    next if (!($globals{outputs}{$output}{useratts} =~ /suppress_portlist/));
...
foreach $flag (sort keys %flags) {
  if($TMR){
      foreach $sfx (@tmr_sfx) {
        &print($indent,&create_tmr_wirereg_statement("next_",$sfx,$flag,$max_io_length + length("reg ")));
      }
  } else {
    $line = &create_wirereg_statement("",$flag,";",$max_io_length + length("reg next_"));
...
#######################
# if TMR with error detection option is selected
# add 'wire' lines for all error detection signals
#######################
if($TMR && $err_det){
  foreach $output (sort keys %{ $globals{outputs} }) {
    ($var = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
    foreach $sfx (@tmr_sfx) {
      &print($indent,"$tmr_sig_attr wire err_det_$var$sfx;\n");
    }
  }
  # add a combined error detected signal for an 'OR' of all errors for incrementing the counter 
  foreach $sfx (@tmr_sfx) {
    &print($indent,"$tmr_sig_attr wire err_det$sfx;\n");
  }
}
...
#######################
# For TMR case
# all outputs snd flags assigned to majority logic
# of replicated registers
#######################
if($TMR){
  &print(0, "\n");
  &print($indent,"// Assignment of outputs and flags to voted majority logic of replicated registers\n");
  foreach $output (sort keys %{ $globals{outputs} }) {
    ($var = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
    if($globals{outputs}{$output}{type} eq "flag" || ($globals{outputs}{$output}{useratts} =~ /suppress_portlist/)){
      foreach $sfx (@tmr_sfx){
      ...
      if(!($globals{outputs}{$output}{useratts} =~ /suppress_portlist/)){
        &print($indent, "assign $var = voted_${var}$tmr_sfx[0];\n");
      }
    } else {
      &print($indent,&vote("",$var,""));
    }
  }
  # If TMR error detection option
  # add error detection logic
  &print(0, "\n");
  &print($indent,"// Assignment of error detection logic to replicated signals\n");
  if($err_det){
    foreach $output (sort keys %{ $globals{outputs} }) {
      ($var = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
      foreach $sfx (@tmr_sfx) {
...
#######################
# For TMR error detection case
# OR all errror detection signals to form one signal for incrementing the error counter
#######################
if($TMR && $err_det){
  &print(0, "\n");
  &print($indent,"// Assign 'OR' of all error detection signals\n");
  foreach $sfx (@tmr_sfx) {
    &print($indent,"assign err_det$sfx = err_det_$statevar$sfx");
    foreach $output (sort keys %{ $globals{outputs} }) {
      ($var = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
      &print(""," | err_det_$var$sfx");
    }
    &print("",";\n");
  }
}
...
# Setup replication loop for -TMR option
if($TMR) {
  @tmr_loop = @tmr_sfx;
  $prfx = "voted_";
} else {
...
foreach $sfx (@tmr_loop){
  if ($encoding eq "heros") {
    ...
    if ($globals{machine}{implied_loopback}{value}) {
      &print($indent, "$nextstatevar$sfx = $prfx$statevar$sfx; // default to hold value because implied_loopback is set\n");
  ...
  #      &print($indent, sprintf("$nextstatevar = ${total_statebits}\'b%s; // default to x because default_state_is_x is set\n","x"));
	&print($indent, sprintf("$nextstatevar$sfx = ${total_statebits}\'b%s; // default to x because default_state_is_x is set\n","x" x ${total_statebits}));
      } elsif ($language eq "systemverilog") {
	&print($indent, sprintf("$nextstatevar$sfx = XXX; // default to x because default_state_is_x is set\n"));
      }
    } else {
  ...
  #  }
    if ($globals{machine}{default_state_is_x}{value}) {
      ...
      &print($indent, sprintf("$nextstatevar$sfx = ${total_statebits}\'b%s;\n","0" x $total_statebits));
    }
  }
}
...
foreach $combout (sort keys %comb_outputs) {
  foreach $sfx (@tmr_loop){
    ($cmbout = $combout) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
    $cmbout = "$cmbout$sfx"; # add replicator suffix if any
    if (exists $comb_outputs{$combout}{default_value} && ($comb_outputs{$combout}{default_value} ne "") ) {
      ...
      #check default values for references to suppressed outputs and replace with replicated version
      $cmb_default = $comb_defaults{$combout};
      foreach $output (sort keys %{ $globals{outputs} }) {
        ($temp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
         if($globals{outputs}{$output}{useratts} =~ /suppress_portlist/ && $temp eq $cmb_default){
            $cmb_default =~ s/$temp/${prfx}${temp}${sfx}/g;
         }
      }
      &print($indent,"$cmbout = $cmb_default; // default\n");
    } else {
      $comb_defaults{$combout} = "0";  # Use string zero
      &warning($indent,"Combinational output $combout has no default value - using 0");
      &print($indent,"$cmbout = $comb_defaults{$combout}; // default to zero for better synth results (no default set in .fzm file)\n");
...
foreach $flag (sort keys %flags) {
  foreach $sfx (@tmr_loop){
    ($flg = $flag) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
    $flag_defaults{$flag} = $flags{$flag}{default_value};
    &print($indent,"next_${flg}$sfx = $prfx$flg$sfx;\n");
  }
}

foreach $sfx (@tmr_loop){
  # State "case" block
  &print_case_statement("$prfx$statevar$sfx");
  ...
  foreach $state (@allstates) {
    ...
    if ($encoding eq "heros") {
      ...
      if ($language eq "systemverilog" || ($language eq "verilog" && $globals{machine}{default_state_is_x}{value})) {
	&print($indent++,sprintf("%-${statename_length_onehot}s:","$prfx$statevar$sfx\[${state}_BIT\]") . " begin${comment}\n");
      } elsif ($language eq "verilog") {
	&print($indent++,sprintf("%-${statename_length_onehot}s:","$prfx$statevar$sfx\[$state\]") . " begin${comment}\n");
      }
    }
    ...
    foreach $comb_output (sort keys %comb_outputs) {
      ($cmb_output = $comb_output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
      $cmb_output = "$cmb_output$sfx"; # add replicator suffix if any
	...
	$value = $comb_outputs{$comb_output}{default_value};
      ...
      #check values for references to flags and suppressed outputs and replace with replicated version
      $rep_value = $value;
      foreach $output (sort keys %{ $globals{outputs} }) {
        ($temp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
         if($globals{outputs}{$output}{type} eq "flag" || $globals{outputs}{$output}{useratts} =~ /suppress_portlist/){
            $rep_value =~ s/$temp/${prfx}${temp}${sfx}/g;
         }
      }
	...
	if ($in_flag_on_transitions_mode) {
	  &warning($indent,"Flag $flag is assigned on transitions, but is also assigned value \"$states{$state}{attributes}{$flag}{value}\" in state $state");
        ...
        #check values for references to flags and suppressed outputs and replace with replicated version
        $rep_flag_value = $states{$state}{attributes}{$flag}{value};
        foreach $output (sort keys %{ $globals{outputs} }) {
          ($temp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
           if($globals{outputs}{$output}{type} eq "flag" || $globals{outputs}{$output}{useratts} =~ /suppress_portlist/){
              $rep_flag_value =~ s/$temp/${prfx}${temp}${sfx}/g;
           }
        }
	# output assignment statement
	&print($indent,"next_${flg}$sfx = $rep_flag_value;\n");
      }
    }
    ...
    for (my $i = 0; $i<=$#transitions_from_this_state; $i++) {
      ...
      $equation = $transitions{$trans}{attributes}{equation}{value};
      $equation =~ s/\n//g; # remove embedded cr's
      #check equation for references to flags and suppressed outputs and replace with replicated version
      foreach $output (sort keys %{ $globals{outputs} }) {
        ($temp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
         if($globals{outputs}{$output}{type} eq "flag" || $globals{outputs}{$output}{useratts} =~ /suppress_portlist/){
            $equation =~ s/$temp/${prfx}${temp}${sfx}/g;
         }
      }
	...
	&print($indent,"$nextstatevar$sfx = $nextstate;$comment\n");
      } elsif ($encoding eq "onehot") {
	if ($language eq "systemverilog" || ($language eq "verilog" && $globals{machine}{default_state_is_x}{value})) {
	  &print($indent,"$nextstatevar$sfx = $nextstate;$comment\n");
	} elsif ($language eq "verilog") {
	  &print($indent,"$nextstatevar$sfx\[$nextstate\] = 1'b1;$comment\n");
	}
      }
	...
	if ($transitions{$trans}{attributes}{$att}{type} eq "output") {
         ($cf_out = $att) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
	  if (exists $comb_outputs{$att}) {
	    ...
	    &error($indent,"Value of comb output $att cannot be determined on transition $trans") if ($value eq "");
            #check values for references to flags and suppressed outputs and replace with replicated version
            $rep_trns_value = $value;
            foreach $output (sort keys %{ $globals{outputs} }) {
              ($temp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
              if($globals{outputs}{$output}{type} eq "flag" || $globals{outputs}{$output}{useratts} =~ /suppress_portlist/){
                  $rep_trns_value =~ s/$temp/${prfx}${temp}${sfx}/g;
              }
            }
		...
		&print($indent,"$cf_out$sfx = $rep_trns_value;\n");
	    }
	  }
	  if (exists $flags{$att}) {
	    # flags
	    if ($transitions{$trans}{attributes}{$att}{value} ne "") {
              #check values for references to flags and suppressed outputs and replace with replicated version
	      $rep_trns_value = $transitions{$trans}{attributes}{$att}{value};
              foreach $output (sort keys %{ $globals{outputs} }) {
                ($temp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
                if($globals{outputs}{$output}{type} eq "flag" || $globals{outputs}{$output}{useratts} =~ /suppress_portlist/){
                    $rep_trns_value =~ s/$temp/${prfx}${temp}${sfx}/g;
	      ...
	      &print($indent,"next_${cf_out}$sfx = $rep_trns_value;\n");
	    }
	  }
	}
      }
	...
	if ($globals{machine}{implied_loopback}{value}) {
	  ...
	  if ($language eq "systemverilog") {
	    &print($indent,"$nextstatevar$sfx = $state; // Added because implied_loopback is true\n");
	  } elsif ($language eq "verilog") {
	    &print($indent,"$nextstatevar$sfx\[$state\] = 1'b1; // Added because implied_loopback is true\n");
	  }
	  if ($no_transitions_from_this_state) {
	  } else {
	    &print(--$indent,"end\n"); 
  ...
  if ($use_undefined_goto_state) {
    ...
    if ($encoding eq "heros") {
      &print(++$indent,"$nextstatevar$sfx = $undefined_goto_state; // Added because undefined_states_go_here is set\n");
    } elsif ($encoding eq "onehot") {
	  if ($language eq "systemverilog" || $globals{machine}{default_state_is_x}{value}) {
	    &print(++$indent,"$nextstatevar$sfx = $undefined_goto_state; // Added because undefined_states_go_here is set\n");
	  } elsif ($language eq "verilog") {
	    &print(++$indent,"$nextstatevar$sfx\[$undefined_goto_state\] = 1'b1; // Added because undefined_states_go_here is set\n");
	  }
    }
    &print(--$indent, "end\n");
  } elsif ($casedefault) {
...
unless ($encoding eq "onehot") {
  ...
  foreach $regout (sort {
    ...
    # for TMR'd "reg" outputs use the first voted state.
    if($TMR){
      $vtprfx = "voted_";
      $sfx = $tmr_sfx[0];
    } else {
    ...
    if ($total_statebits == 1) {
      &print($indent,"assign $regout = ${vtprfx}$statevar${sfx};\n");
    } else {
      &print($indent,"assign $regout = ${vtprfx}$statevar${sfx}\[$reg_outputs{$regout}{statebit_range}\];\n");
    }
  }
  &print(0,"\n");
}
...
if ($reset_signal) {
  ...
  foreach $sfx (@tmr_loop){
    if ($encoding eq "heros") {
      &print($indent,"$statevar$sfx <= $reset_state;\n");

    } elsif ($encoding eq "onehot") {
      if ($language eq "systemverilog" || ($language eq "verilog" && $globals{machine}{default_state_is_x}{value})) {
	&print($indent,"$statevar$sfx <= $reset_state;\n");
      } elsif ($language eq "verilog") {
	&print($indent,"$statevar$sfx <= $zero_padded_one << $reset_state;\n");
      }
    }
  }
  if($err_det) {
    foreach $sfx (@tmr_loop){
      &print($indent,"ed_cnt$sfx <= 0;\n");
    }
  }
  # Add flags
  foreach $flag (sort keys %flags) {
...
$indent++;
foreach $sfx (@tmr_loop){
  &print($indent,"$statevar$sfx <= $nextstatevar$sfx;\n");
}
if($err_det) {
  foreach $sfx (@tmr_loop){
    &print($indent,"ed_cnt$sfx <= err_det$sfx ? voted_ed_cnt$sfx + 1 : voted_ed_cnt$sfx;\n");
  }
}
# Add flags
foreach $flag (sort keys %flags) {
  foreach $sfx (@tmr_loop){
    ($flg = $flag) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
    &print($indent,"$flg$sfx <= next_${flg}$sfx;\n");
  }
}
$indent--;
# Add end if there are flags
...
if (scalar(%regdp_outputs)) {
  ...
  if ($reset_signal) {
    ...
    foreach $regdp_output (sort keys %regdp_outputs) {
      ...
      foreach $sfx (@tmr_loop){
        ($rgdp = $regdp_output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
        #check values for references to flags and suppressed outputs and replace with replicated version
        foreach $output (sort keys %{ $globals{outputs} }) {
          ($temp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
          if($globals{outputs}{$output}{type} eq "flag" || $globals{outputs}{$output}{useratts} =~ /suppress_portlist/){
            $value =~ s/$temp/${prfx}${temp}${sfx}/g;
          }
        }
        &print($indent,"$rgdp$sfx <= $value;\n");
      }
    }
    &print(--$indent,"end\n");
    &print($indent++,"else begin\n");
  ...
  foreach $regdp_output (sort keys %regdp_outputs) {
	...
	$dp_seq_defaults{$regdp_output} = $regdp_outputs{$regdp_output}{default_value};
        #check default values for references to suppressed outputs and replace with replicated version
        $rep_regdp_dflt = $dp_seq_defaults{$regdp_output};
        foreach $output (sort keys %{ $globals{outputs} }) {
          ($temp = $output) =~ s/\s*\[\d+:\d+\]\s*//; # get rid of range field
           if($globals{outputs}{$output}{useratts} =~ /suppress_portlist/ && $temp eq $rep_regdp_dflt){
              $rep_regdp_dflt =~ s/$temp/${prfx}${temp}${sfx}/g;
           }
        }
	&print($indent,"$rgdp$sfx <= $rep_regdp_dflt; // default\n");
      } else {
	$dp_seq_defaults{$regdp_output} = "0";  # Use string zero
	&warning($indent,"Datapath output $regdp_output has no default value - using 0");
	&print($indent,"$rgdp$sfx <= $dp_seq_defaults{$regdp_output}; // default to zero for better synth results (no default set in .fzm file)\n");
  ...
  foreach $sfx (@tmr_loop){
    # State "case" block
    &print_case_statement("$nextstatevar$sfx");
	...
	if ($language eq "systemverilog" || ($language eq "verilog" && $globals{machine}{default_state_is_x}{value})) {
	  &print($indent++,sprintf("%-${nextstatename_length_onehot}s:","$nextstatevar$sfx\[${state}_BIT\]") . " begin\n");
	} elsif ($language eq "verilog") {
	  &print($indent++,sprintf("%-${nextstatename_length_onehot}s:","$nextstatevar$sfx\[$state\]") . " begin\n");
	}
      }
...
if ($simcode) {
  ...
  if($TMR) { # if TMR is selected the statevar and nextstatevar names have a suffix
    $sfx = $tmr_sfx[0];
  } else {
    $sfx = "";
  }
  # State "case" block
  if ($encoding eq "heros") {
    &print($indent++,"case ($statevar$sfx)\n");
  } elsif ($encoding eq "onehot") {
  ...
  foreach $state (@allstates) {
    if ($encoding eq "heros") {
      ...
      if ($language eq "systemverilog" || ($language eq "verilog" && $globals{machine}{default_state_is_x}{value})) {
        &print($indent++,sprintf("%-${statename_length_onehot}s:","$statevar$sfx\[${state}_BIT\]") . "\n");
      } elsif ($language eq "verilog") {
        &print($indent++,sprintf("%-${statename_length_onehot}s:","$statevar$sfx\[$state\]") . "\n");
      }
    }
...
# Create the wire/reg/logic statements for the replicated outputs (TMR case).
# Routine replaces create_wirereg_statement for the TMR case.
# argument is output to create statement for.
# returns statement.  Also sets range info globally.
sub create_tmr_wirereg_statement {
  my ($prfx,$rep,$output,$statement_length) = (@_);

  # set synthesis attribute
  if ($rep eq "") {
    # original output signals are not replicated and do not need a synthesis attribute
    $tmr_attr = "";
  } elsif ($prfx eq "voted_" || $prfx eq "next_" || $prfx eq "err_det_" || $prfx eq "err_det") {
    # flags get extra replicated "voted" signals which are "wires"
    $tmr_attr = $tmr_sig_attr;
  } else {
    $tmr_attr = $tmr_reg_attr;
  }
  ...
  # Handle comments
  if ($globals{outputs}{$output}{comment} && $rep eq "") {
    $comment = "// $globals{outputs}{$output}{comment}";
  } else {
    $comment = "";
  }
  if ($language eq "verilog") {
    # All original outputs are type "wire" and will have a continuous assignment to the voted result
    # All "reg" type outputs will not be replicated but will have a continuous assignment to <voted_state_1>
    # All other outputs will be replicated and instances and will be sequential logic like "regdp" 
    if ($rep eq "") {
      $type = "wire";
    } elsif ($prfx eq "voted_" || $prfx eq "err_det_" || $prfx eq "err_det") {
      # flags get extra replicated "voted" signals which are "wires"
      $type = "wire";
    } else {
      $type = "reg";
  ...
  # Handle multibit outputs
  if (($name,$range) = ($output =~ /^([^\[]+)\s*(\[.*\])/)) {
    $line = sprintf("%-${max_attr_length}s %-${statement_length}s %-s\n",$tmr_attr,"$type $range ${prfx}$name${rep};",$comment);
    ($upper,$lower) = $range =~ /(\d+):(\d+)/;
    $widths{$name} = abs($upper - $lower) + 1;
  } else {
    $line = sprintf("%-${max_attr_length}s %-${statement_length}s %-s\n",$tmr_attr,"$type ${prfx}$output${rep};",$comment);
    $widths{$output} = 1;
  }
  # Remove trailing blanks
  $line =~ s/\s*\n$/\n/;
...
# Create continuous assignment for voting replicated logic
sub vote {
  my ($prfx, $var, $sfx) = (@_);
  my ($max1, $max2);
  $max1 = $max_outflag_name_length + length($prfx) + length($sfx);
  $max2 = $max_output_name_length + 2;
  if($language eq "verilog" || $language eq "systemverilog"){
    $line = sprintf("assign %-${max1}s = (%-${max2}s & %-${max2}s) | (%-${max2}s & %-${max2}s) | (%-${max2}s & %-${max2}s); // Majority logic\n",
                    "${prfx}${var}$sfx", "${var}$tmr_sfx[0]", "${var}$tmr_sfx[1]", "${var}$tmr_sfx[1]", "${var}$tmr_sfx[2]", "${var}$tmr_sfx[0]", "${var}$tmr_sfx[2]");
  }
  return($line);
}

# Create continuous assignment for error detection logic
sub errdet {
  my ($prfx, $var, $sfx) = (@_);
  my ($max1, $max2);
  $max1 = $max_outflag_name_length + length($prfx) + length($sfx);
  $max2 = $max_output_name_length + length($sfx);
  if($widths{$var} eq "1"){
    $r_op = " ";
  }
  else {
  ...
  if($language eq "verilog" || $language eq "systemverilog"){
    $line = sprintf("assign %-${max1}s = $r_op(~((~%-${max2}s & ~%-${max2}s & ~%-${max2}s) | (%-${max2}s & %-${max2}s & %-${max2}s))); // error detection logic\n",
                    "${prfx}${var}$sfx", "${var}$tmr_sfx[0]", "${var}$tmr_sfx[1]", "${var}$tmr_sfx[2]", "${var}$tmr_sfx[0]", "${var}$tmr_sfx[1]", "${var}$tmr_sfx[2]");
  }
  return($line);
}
...
sub process_options {
  ...
  die unless GetOptions(
    ...
    "TMR!" => \$TMR, # bgb added triple modular redundency option
    "synth_attr=s" => \$synth_attr, # Specify use of vendor specific synthesis attributes for TMR option.    
    "err_det!" => \$err_det, # bgb added error detection option for TMR
    "tmr_err_count=s" => \$tmr_err_count, # Specify name of error count output variable for TMR option only.    
    );
...
sub show_help {
 print STDERR "Syntax:
  ...
  -TMR                  Use Triple Modular Redunancy (TMR) for mitigating Single Event Upsets (SEU)
                        Replicates state registers and regdp's as well as combinitorial logic.
  -synth_attr <value>   Specify vendor target for synthesis attributes.  For use with -TMR option to prevent
                        the sysnthesis tool from optimizing away replication.
                        Options are XST (Xilinx Sythesis Tool), synplify (Synopsys Synplify), or precision (Mentor Graphics Precision RTL)
  -err_det              Include error detection with TMR option.
  -tmr_err_count <name> Use <name> instead of \"TMR_ERR_COUNT\" for TMR error count output variable.
